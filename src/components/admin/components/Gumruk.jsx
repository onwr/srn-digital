...
const Gumruk = () => {
  ...
  useEffect(() => {
    const veriCek = async () => {
      try {
        const qDoc = collection(db, "musavirlikler");
  ...
  const acenteSil = async (id) => {
    try {
      await deleteDoc(doc(db, "musavirlikler", id));
  ...
  return (
    <div className="container max-w-screen-lg border bg-zinc-100 p-4 rounded-xl">
      <p className="text-xl font-semibold">Gümrük Müşavirlikleri</p>
