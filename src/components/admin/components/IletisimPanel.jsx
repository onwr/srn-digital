...
const IletisimPanel = () => {
    ...
    return (
        <div className='duration-300 container max-w-screen-lg w-full border bg-zinc-100 p-4 rounded-xl'>
            <p className='text-xl font-semibold'>İletişim Bilgilerini Güncelle</p>
            <form className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-3' onSubmit={handleSubmit}>
                {['adres', 'mail', 'tel'].map((field, index) => (
                    <input 
                        key={index}
                        type="text" 
                        name={field} 
                ...
                <button type="submit" className='col-span-1 md:col-span-2 p-2 bg-lime-400 text-white rounded-lg'>Kaydet</button>
            </form>
        </div>
    );
};

export default IletisimPanel;
